# Task 002: STM32 UART配置修复

## 目标
根据Task 001的诊断结果，修复STM32 USART2配置问题，使TX正常输出数据。
...
static void uart_gpio_init(uart_instance_t instance)
{
    ...
    if (instance == UART_INSTANCE_1) {
        ...
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;  // 或 GPIO_MODE_AF_OD
        gpio_init_struct.Pull = GPIO_PULLUP;
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;
        gpio_init_struct.Alternate = GPIO_AF7_USART2;  // 确认使用AF7
        HAL_GPIO_Init(GPIOA, &gpio_init_struct);
...
```c
    ...
    /* 确保USART使能 - HAL_UART_Init应该已设置，但双重确认 */
